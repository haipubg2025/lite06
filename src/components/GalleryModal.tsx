...
export default function GalleryModal({ onClose, isSelectMode, onSelect }: GalleryModalProps) {
  ...
  return (
    ...
    >
      <div 
        className={`w-full h-full flex flex-col rounded-none border-0 overflow-hidden relative shadow-none ${isDark ? 'bg-[#050505]' : 'bg-[#FAF7F0]'}`}
      ...
      {/* Header */}
      <div className={`p-4 md:p-6 flex items-center justify-end shrink-0 relative z-10 border-b ${isDark ? 'border-white/5 bg-black/40' : 'border-amber-200 bg-[#FFFDF9]/60 backdrop-blur-sm'}`}>
        ...
        <div className="absolute inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowUrlInput(false)}>
          <div 
            className={`rounded-2xl p-6 max-w-md w-full shadow-2xl flex flex-col gap-4 border ${
              isDark ? 'bg-[#111] border-white/10' : 'bg-[#FFFDFB] border-amber-250'
            }`}
            onClick={(e) => e.stopPropagation()}
          >
