...
export default function Sidebar({ onMobileSelect }: SidebarProps) {
  ...
  return (
    <div className={`h-full w-full flex flex-col ${theme.sidebarClass}`}>
        ...
        <div className="p-8">
          ...
          >
            ...
            <h1 className={`text-xl font-bold tracking-tighter ${theme.textPrimary}`}>
              MATRIX <span className={(theme.accentClass || 'text-white border-white').split(' ')[0]}>LITE v2</span>
            </h1>
          </motion.div>
      ...
      <div className={`mt-auto p-8 border-t space-y-4 ${theme.group === 'Dark' ? 'border-white/5' : 'border-amber-200/30'}`}>
        {/* Lựa chọn giao diện */}
        <div className={`flex p-1 rounded-xl border border-transparent shadow-none ${theme.group === 'Dark' ? 'bg-black/40' : 'bg-[#FFFDFB]/80 border-amber-200/50 shadow-sm'}`}>
          <button 
            onClick={() => setUiMode('auto')}
            className={`flex-1 flex justify-center items-center py-2 rounded-lg transition-all ${uiMode === 'auto' ? (theme.group === 'Dark' ? 'bg-white/20 text-white shadow' : 'bg-amber-600 text-white shadow font-bold') : (theme.group === 'Dark' ? 'text-white/40 hover:text-white/70 hover:bg-white/5' : `text-[#5C4033]/80 hover:bg-amber-500/10 hover:text-[#3E2723]`)}`}
            title="Auto Detect"
          >
          ...
          <button 
            ...
            className={`flex-1 flex justify-center items-center py-2 rounded-lg transition-all ${uiMode === 'pc' ? (theme.group === 'Dark' ? 'bg-white/20 text-white shadow' : 'bg-amber-600 text-white shadow font-bold') : (theme.group === 'Dark' ? 'text-white/40 hover:text-white/70 hover:bg-white/5' : `text-[#5C4033]/80 hover:bg-amber-500/10 hover:text-[#3E2723]`)}`}
            title="PC Mode"
          >
          ...
          <button 
            ...
            className={`flex-1 flex justify-center items-center py-2 rounded-lg transition-all ${uiMode === 'mobile' ? (theme.group === 'Dark' ? 'bg-white/20 text-white shadow' : 'bg-amber-600 text-white shadow font-bold') : (theme.group === 'Dark' ? 'text-white/40 hover:text-white/70 hover:bg-white/5' : `text-[#5C4033]/80 hover:bg-amber-500/10 hover:text-[#3E2723]`)}`}
            title="Mobile Mode"
          >
